...
# --- MODIFIED: Added output_dir parameter ---
def generate_certificate(device_info: dict, private_key, output_dir: str):
    """Creates and signs a certificate, saving files to the specified directory."""
    timestamp_str = datetime.datetime.utcnow().strftime("%Y%m%d-%H%M%S")
    
    # Create file paths in the specified output directory
    ...
    # Return the full paths of the created files
...
def save_pdf_from_json(cert_data: dict, pdf_filename: str, verification_url: str):
    ...
    link_rect = (link_x, link_y, link_x + text_width, link_y + 10)
...
def verify_certificate_data(cert_data: dict, public_key):
    """Verifies a certificate from a dictionary object. Returns (bool, message)."""
    try:
        cert = dict(cert_data)
