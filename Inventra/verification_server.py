...
# --- NEW: LOAD PUBLIC KEY ON STARTUP ---
try:
    PUBLIC_KEY = key_manager.load_public_key()
except FileNotFoundError:
    print("FATAL ERROR: 'public.pem' not found. Server cannot start.", file=sys.stderr)
    sys.exit(1)
except Exception as e:
    print(f"FATAL ERROR: Could not load public key: {e}", file=sys.stderr)
    sys.exit(1)
# --- END NEW ---
...
def register_wipe_api():
    ...
    # --- NEW: VERIFY SIGNATURE BEFORE REGISTERING ---
    is_valid, message = verify_certificate_data(cert_data, PUBLIC_KEY)
    
    if not is_valid:
        # If the certificate is fake, reject it.
        return jsonify({"status": "error", "message": f"Certificate verification failed: {message}"}), 400
    # --- END NEW ---

    # If we are here, the certificate is valid. Proceed to save.
    try:
        conn = sqlite3.connect(DATABASE_FILE)
        cursor = conn.cursor()
        cursor.execute(
        ...
        conn.commit()
        conn.close()
        return jsonify({"status": "success", "message": "Certificate verified and registered."}), 201
    except sqlite3.IntegrityError:
        # This error happens if the certificate_id is already in the DB
        return jsonify({"status": "error", "message": "This certificate ID has already been registered."}), 409
    except Exception as e:
...
def verify_certificate_web(certificate_id):
    ...
    if record:
        ...
        <html><body style='font-family: sans-serif; text-align: center; padding: 20px;'>
            ...
            <p style='color: #555; margin-top: 20px;'>This certificate has been cryptographically verified and registered by the server.</p>
        </body></html>
        """
...
if __name__ == '__main__':
    init_db()
    # Note: The app will now crash on startup if public.pem is missing,
